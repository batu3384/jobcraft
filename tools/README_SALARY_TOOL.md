...
Edit `salary_data.json` with real benchmarks; the file is gitignored.

## How it works

The tool reads a `salary_data.json` file in the repo root containing company salary benchmarks. It uses fuzzy matching to find companies by name, handling Turkish/English company names, legal suffixes (A.Ş., Ltd. Şti., A/S), and common spelling variations.
...
```json
{
  "metadata": {
    ...
    "index_baseline": 0,
    "index_label": "Monthly salary (TRY)",
...
```bash
python3 salary_lookup.py "Trendyol"
python3 salary_lookup.py "Turkcell" --city "İstanbul"
python3 salary_lookup.py "Softtech" --json
