...
Usage:
    ...
    python salary_lookup.py "Company Name" --city "İstanbul"
...
SAMPLE_FILE = Path(__file__).parent / "salary_data.sample.json"

# Turkish / Latin diacritic folding for fuzzy company-name match
SPELLING_VARIANTS = {
    "ø": "o", "æ": "ae", "å": "aa",
    "ö": "o", "ä": "ae", "ü": "u",
    "ğ": "g", "ş": "s", "ı": "i", "ç": "c",
...
def normalize_salary_data(data):
    """Coerce shorthand category numbers to {index: n} objects."""
    if not isinstance(data, dict):
        return data
    companies = data.get("companies")
    if not isinstance(companies, list):
    ...
    for entry in companies:
        ...
        if not isinstance(categories, dict):
            continue
        for cat_label, cat_data in list(categories.items()):
            if isinstance(cat_data, (int, float)):
                categories[cat_label] = {"index": cat_data}
...
def read_raw_data():
    """Load and JSON-parse salary_data.json; fall back to sample with a warning."""
    path = DATA_FILE
    using_sample = False
    if not path.exists():
        if SAMPLE_FILE.exists():
            ...
            print("", file=sys.stderr)
            print("Copy salary_data.sample.json to salary_data.json or run ./install.sh", file=sys.stderr)
    ...
    if using_sample:
        print(
            "Note: using salary_data.sample.json (synthetic TRY indices). "
            "Copy to salary_data.json and replace with real benchmarks.",
            file=sys.stderr,
        )
    return data
...
def load_data():
    ...
    return validate_data(normalize_salary_data(read_raw_data()))
...
def anglicize(s):
    """Fold Turkish/Latin diacritics for fuzzy matching."""
    s = s.lower()
